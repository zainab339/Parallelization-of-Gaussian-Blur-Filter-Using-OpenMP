...
void total_blur(int *in, int *out, int w, int h, int r)
{
    ...
    for (int i = 0; i < w; i++)
    {
        ...
        // Thread-local logs and global error tracker
        std::vector<std::ostringstream> thread_logs(omp_get_max_threads());
        std::atomic<bool> success(true); // Track if parallelism succeeded
...
#pragma omp parallel for reduction(+ : val)
        for (int j = 0; j <= r; j++)
        {
            ...
            {
                success.store(false); // Mark parallelism as failed
                thread_logs[thread_id] << "Thread " << thread_id << " - Error: " << e.what() << "\n";
            }
        }

        // Print all thread logs sequentially
        for (size_t t = 0; t < thread_logs.size(); ++t)
        {
            std::cout << thread_logs[t].str();
        }
        ...
        if (!success.load())
        {
            std::cerr << "Parallelism encountered errors! Check thread logs for details.\n";
        }
        else
        {
            std::cout << "Parallelism completed successfully.\n";
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    int image_count = 0;
    float total_elapsed = 0.0f, total_io = 0.0f, total_preprocessing = 0.0f;
